...
const ProductSets = () => {
  ...
  useEffect(() => {
    fetch('data/ProductSkinSets.json')
      .then((response) => response.json())
      .then((data) => {
        const tempList = data.map((product) => (
          <div key={product.id} className="col-12 col-sm-6 col-md-4 col-lg-3 mb-4">
  <div className="card my-5">
    {/* Optionally, you can add a badge for offers or sales */}
    {/* <span className="onsale product-image-badge bg-warning text-white text-center w-25 position-absolute rounded-2">Sets!</span> */}
    <div className="card-body p-0 bg-light">
      <img className="w-100 m-0 rounded-top-2" src={product.image} alt={product.name} />
    </div>
    <div className="card-footer text-centerz">
